...
var ants = (function() {
	...
	return {
		...
		init : function() {
			...
			var TRAIL_STRENGTH_DECAY = 0.00005;
			...
			var ant = function(myHome, team) {
				...
				return {
					...
					gender: gender,
					hasFood: false,
					x: myHome.entrances[startEnt].x,
					y: myHome.entrances[startEnt].y,
					width: 3,
					height: 3,
					...
					for(var n = 0; n < foodlen; n++) {
						...
						if(ants.foodData[n] > 0) {
						 	if((Math.random() * 1000 + 1) < 25) {
						    	...
						    	ctxf.fillRect(Math.floor((n) % CANVAS_WIDTH),Math.floor(n / CANVAS_WIDTH),2,2);
								drawFood(Math.floor((n) % CANVAS_WIDTH),Math.floor(n / CANVAS_WIDTH),2,2);
							}
						}
					}
